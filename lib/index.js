...
import { access, lstat, open, readFile, readdir, realpath, unlink } from "node:fs/promises";
...
const redoSnapshots = new Map();
function redoKey(cwd, path) {
	return cwd + "\0" + path;
}
function rememberRedo(cwd, snapshot) {
...
async function resolveWritableTarget(cwd, requestedPath) {
	if (typeof cwd !== "string" || cwd === "") throw new Error("cwd is required");
	if (typeof requestedPath !== "string" || requestedPath === "") throw new Error("path is required");
	const root = await realpath(cwd);
	const candidate = resolve(root, requestedPath);
...
async function redoFromHunks(cwd, file) {
	const target = await resolveWritableTarget(cwd, file.path);
	if (!target.missing && target.size > UNDO_TEXT_LIMIT) return {
		path: file.path,
		ok: false,
		error: "file too large to restore safely (" + target.size + " bytes)"
	};
	...
	for (const hunk of file.diffs) {
		...
		if (hunk.oldText === null) {
			...
			if (next === null) return {
				path: file.path,
				ok: false,
				error: "file drifted: expected prior text not found or ambiguous"
			};
			text = next;
		}
	}
...
async function redoFile(cwd, file) {
	try {
		...
		if (snapshot === void 0) {
			if (!Array.isArray(file.diffs) || file.diffs.length === 0) return {
				path: file.path,
				ok: false,
				error: "no undo snapshot to restore"
			};
			return await redoFromHunks(cwd, file);
		}
		const target = await resolveWritableTarget(cwd, file.path);
		if (!target.missing && target.filename !== snapshot.filename) return {
			path: file.path,
			ok: false,
			error: "file changed while being accessed (link swap)"
		};
		if (!target.missing) {
			if ((await readWhole(target)).text === snapshot.text) {
...
async function resolveVisualStudioCodeCli() {
	for (const bin of await listPathCommands("code")) try {
...
async function darwinVisualStudioCodeApp() {
	const home = process.env.HOME ?? "";
	const candidates = ["/Applications/Visual Studio Code.app", home !== "" ? join(home, "Applications/Visual Studio Code.app") : ""].filter((entry) => entry !== "");
	for (const app of candidates) try {
		await access(app);
		return app;
	} catch {
		continue;
